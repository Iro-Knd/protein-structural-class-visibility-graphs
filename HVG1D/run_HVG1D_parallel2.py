...
def compute_graph_metrics(graph):
    # Calculating HVG graph features
    metrics = {
...
def process_matrix(matrix):
    ...
    # Creation HVG
    hvg_x = hvg_1d(x_values)
    hvg_y = hvg_1d(y_values)

    # hvg_x = fast_hvg_graph(x_values, shiftvalue=0.0)
    # hvg_y = fast_hvg_graph(y_values, shiftvalue=0.0)
...
def main():
    ...
    # df = pd.DataFrame(results, columns=headerList) # if you want headers
    # Dataframe without headers
    df = pd.DataFrame(results)
    df.to_excel("101HVG_1D_Results_Parallel.xlsx", index=False, header=False)
    print("Results stored at HVG_1D_Results_Parallel_exceptions.xlsx")
