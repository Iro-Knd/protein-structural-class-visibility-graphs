...
from VG2D_AND_all import *  # must provide: visibility_graph2D_and
from assortativity import assortativity_bct_undirected
from chaos_try2 import chaos_game_representation
# from Average_Shortest_Path import *   # not needed anymore for ASPL
...
def laplacian_energy(graph: nx.Graph) -> float:
    ...
    return float(sum(abs(ev - average_degree) for ev in eigenvalues))
...
def compute_graph_metrics(graph: nx.Graph) -> dict:
    ...
    metrics = {
        ...
        "energy": float(np.sum(np.abs(LA.eigvalsh(nx.to_numpy_array(graph))))),
...
def process_matrix(matrix):
    g2d = visibility_graph2D_and(matrix)
...
def main():
    ...
    out_file = "104VG_2D_AND_Results_Parallel.xlsx"
